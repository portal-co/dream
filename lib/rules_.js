...
function ccLibrary(opts) {
    buildRules += [function (x) {
        ...
        if (x == opts.name) {
            ...
            return makeGob([{ id: '@CCInfo', archive: exec("ar crus $(find .) > '" + opts.name + ".a'", objFiles, [opts.name + ".a"],`Link CXX Library ${x}`)[opts.name + ".a"]}]);
        };
        var y = x.substring(opts.name.length);
        y = y.substring(1);
        return makeGob([{id: '@DefaultInfo', files: exec("ar x ./*.a;cc " + y + " -c -o " + y + ".o" + opts.cflags,Object.assign({},srcFiles,depFiles),[y + ".o"],`Compiling CXX object ${x}`)}])
    }]
}