...
function ninjaLibrary(opts) {
  ...
  buildRules += [function (x) {
    ...
    var _iterator4 = _createForOfIteratorHelper(rules[x].deps),
        _step4;
...
function ccLibrary(opts) {
  buildRules += [function (x) {
    ...
    var _iterator5 = _createForOfIteratorHelper(srcs),
        _step5;
    ...
    if (x == opts.name) {
      ...
      var _iterator7 = _createForOfIteratorHelper(objs),
          _step7;
      ...
      return makeGob([{
        id: '@CCInfo',
        archive: exec("ar crus $(find .) > '" + opts.name + ".a'", objFiles, [opts.name + ".a"])[opts.name + ".a"]
      }]);
    }
    ...
    return makeGob([{
      id: '@DefaultInfo',
      files: exec("ar x ./*.a;cc " + y + " -c -o " + y + ".o" + opts.cflags, Object.assign({}, srcFiles, depFiles), [y + ".o"])
    }]);
  }];
}
